...
namespace ISfM
{
    ...
    { // 2892.0,2883.0,823,605 // 1200, 1200, 720, 540
        ...
        std::vector<std::pair<std::pair<int, int>, std::vector<cv::DMatch>>> matchVector(
            Cdate->matchesMap_.begin(), Cdate->matchesMap_.end());
        std::sort(matchVector.begin(), matchVector.end(), compareByVectorSize);
        matchesMap_ = matchVector;
    ...
    Initializer::Returns Initializer::Initialize()
    {
        ...
        int id1=matchesMap_[0].first.first; // 这里的first一定大于second,因为是在创建这个vector的时候决定的
        int id2=matchesMap_[0].first.second;
        // 通过迭代器的指针访问最多特征点vector<cv::DMatch>的值
        std::pair<int, int> keyToFind = std::make_pair(id1, id2);
        std::vector<cv::DMatch> firstVector = findMatch(keyToFind, matchesMap_);
        for (const cv::DMatch &match : firstVector)
        {
            int trainIdx = match.trainIdx; // 获取trainIdx
            int queryIdx = match.queryIdx; // 获取queryIdx
            cv::Point2f point1(frames_[id1]->features_img_[queryIdx]->position_.pt.x,
                               frames_[id1]->features_img_[queryIdx]->position_.pt.y);
            cv::Point2f point2(frames_[id2]->features_img_[trainIdx]->position_.pt.x,
                               frames_[id2]->features_img_[trainIdx]->position_.pt.y);
            points1.push_back(point1);
            points2.push_back(point2);
            pts1.push_back(frames_[id1]->features_img_[queryIdx]); // queryIdx是匹配时的第一张图片
            pts2.push_back(frames_[id2]->features_img_[trainIdx]);
        }
        frames_[id1]->is_registed = true;
        frames_[id2]->is_registed = true;
        ...
        RecoverPoseFromFundanmental(F, pts1, pts2, inlier_mask_F, id1, id2);
    ...
    std::vector<cv::DMatch> Initializer::findMatch(std::pair<int, int> &keyToFind,
    ...
    void Initializer::TriangulateInitPoints(Frame::Ptr &frame_one, Frame::Ptr &frame_two)
    {
        ...
        std::pair<int, int> keyToFind = std::make_pair(frame_one->id_, frame_two->id_);
        std::vector<cv::DMatch> this_match = findMatch(keyToFind, matchesMap_);
        for (size_t i = 0; i < this_match.size(); ++i)
        {
            int queryIdx = this_match[i].queryIdx;// 取出对应的特征点索引
            int trainIdx = this_match[i].trainIdx;
            std::shared_ptr<Feature> queryFeature = frame_one->features_img_[queryIdx];// 根据索引获取特征点
